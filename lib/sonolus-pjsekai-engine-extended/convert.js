...
import { USCColor, USCFade, } from "usctool";
export const uscToLevelData = (usc, offset = 0) => {
    const entities = [];
    const damageNotePositions = new Set();
    // ダメージノートのエンティティを追跡するためのマップ
    const damageNoteEntities = new Map();
    ...
    const append = (intermediate) => {
        ...
        // ダメージノートの場合、位置キーと対応するエンティティをマップに保存
        if (intermediate.archetype === "DamageNote") {
            const beat = intermediate.data[EngineArchetypeDataName.Beat];
            const lane = intermediate.data.lane;
            const size = intermediate.data.size;
            if (typeof beat === "number" && typeof lane === "number" && typeof size === "number") {
                const positionKey = `${beat}:${lane}:${size}`;
                damageNoteEntities.set(positionKey, entity);
            }
        }
    ...
    for (const object of usc.objects) {
        handlers[object.type](object, append, damageNotePositions);
    }
    ...
    // 同じ位置にダメージノートと通常ノートがある場合、ダメージノートを除外
    const filteredEntities = entities.filter(entity => {
        if (entity.archetype === "DamageNote") {
            const beatData = entity.data.find(d => d.name === EngineArchetypeDataName.Beat);
            const laneData = entity.data.find(d => d.name === "lane");
            const sizeData = entity.data.find(d => d.name === "size");
            if (beatData && laneData && sizeData &&
                'value' in beatData && 'value' in laneData && 'value' in sizeData) {
                const positionKey = `${beatData.value}:${laneData.value}:${sizeData.value}`;
                // 同じ位置にダミーフラグがある通常ノートが存在する場合はダメージノートを除外
                if (damageNotePositions.has(positionKey)) {
                    return false;
                }
            }
        }
...
const single = (object, append, damageNotePositions) => {
    const intermediate = {
    ...
    // damageノートと同じ位置かチェック
    const positionKey = `${object.beat}:${object.lane}:${object.size}`;
    if (damageNotePositions?.has(positionKey)) {
        // 同じ位置にダメージノートがある場合は明示的に1を設定
        intermediate.data.dummy = 1;
    }
    if (object.trace) {
...
const damage = (object, append, damageNotePositions) => {
    const intermediate = {
    ...
    // damageノートの位置を記録（beat、lane、sizeのキーを作成）
    const positionKey = `${object.beat}:${object.lane}:${object.size}`;
    damageNotePositions?.add(positionKey);
    append(intermediate);
};
